...
print("Memulai proses ETL Faktual untuk RB Tematik Baru 2025...")
file_path = r'data\raw\Kertas Kerja Evaluasi On Going RB 2025_TW4.xlsx'

# 1. Ekstraksi Data (Menyesuaikan dengan indeks sheet Tematik Baru)
...
df_bersih = pd.DataFrame({
    ...
    'indikator kegiatan utama': df_data[6].ffill(),  # Berasal dari kolom Indikator Permasalahan
    'Rencana Aksi': df_data[9],
    'output(Satuan)': df_data[10],                   # Satuan output
    'pic_pelaksana': df_data[19],                    # Koordinator (PJK)
    'target tw 1': df_data[12],
    'target tw 2': df_data[13],
    'target tw 3': df_data[14],
    'target tw 4': df_data[15],
    'capaian output tw 1': df_data[22],              # Capaian Progres TW1
    'capaian output tw 2': df_data[27],              # Capaian Progres TW2
    'capaian output tw 3': df_data[32],              # Capaian Progres TW3
    'capaian output tw 4': df_data[37],              # Capaian Progres TW4
})
...
for idx, row in df_bersih.iterrows():
    ...
    # Aturan Desimal jika target di TW 4 adalah 100
    if t4 == 100.0:
