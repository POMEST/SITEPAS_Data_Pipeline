...
print("Memulai proses ETL Faktual untuk RB General 2025 (Strict Numeric & Custom Status)...")
...
df_bersih = pd.DataFrame({
    'indikator_utama': df_data[1].ffill(),
    'rencana_aksi_desc': df_data[5],
    'satuan': df_data[6],
...
# 2. Fungsi Ekstrak Angka (Sangat Ketat)
def get_number(val, fallback=0.0):
    ...
    try:
        ...
        # Jika teks murni, otomatis ubah jadi angka target (fallback)
        return float(fallback)

# 3. Proses Transformasi & Pembersihan Total
for tw in range(1, 5):
    target_clean = []
    capaian_persen = []
    status_list = []
    ...
    for i in range(len(df_bersih)):
        ...
        # Normalisasi Skala Persen
        is_percent = (tw4_t == 100.0 or tw4_t == 1.0)
        if is_percent:
            if t > 1.0 and (0.0 < c <= 1.0):
                c = c * 100.0
            elif c > 1.0 and (0.0 < t <= 1.0):
                t = t * 100.0
                
        target_clean.append(t)
        
        # LOGIKA STATUS BARU (Sesuai Permintaan)
        if t == 0:
            if c > 0:
            ...
            # Tetap hitung persentase (dibatasi max 100 untuk visualisasi bar Streamlit)
            pct = (c / t) * 100.0
            capaian_persen.append(min(pct, 100.0))
            
            # Penentuan status spesifik berdasarkan angka aktual
            if c == t:
                status_list.append("Tercapai")
            elif c > t:
                status_list.append("Tercapai Melebihi Target")
            elif c < t:
                status_list.append("Belum Tercapai Pada TW Ini")
