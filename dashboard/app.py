    ...
    with col_chart:
        ...
        if 'status tw 4' in df.columns and 'indikator kegiatan utama' in df.columns:
            ...
            fig = px.bar(
                ...
                barmode='group',
                text_auto=True,
                hover_data={'indikator kegiatan utama': True} # Menampilkan teks penuh saat di-hover
            )
    ...
    with col_insight:
        ...
        with st.container(border=True):
            ...
            if 'Persentase Pencapaian Target' in df.columns:
                ...
                df = df.drop(columns=['pct_num']) # Cleanup

    # Garis pemisah sebelum masuk ke area filter
    st.divider()
...
with tab_general:
    if not df_gen.empty:
        ...
        st.warning("Data RB General belum tersedia.")
...
with tab_tematik:
    if not df_tem.empty:
        ...
        st.warning("Data RB Tematik belum tersedia.")
...
with tab_tematik_baru:
    if not df_tem_baru.empty:
        ...
        st.warning("Data RB Tematik Baru belum tersedia.")